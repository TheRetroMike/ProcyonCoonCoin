...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += ProcyonUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut(AreEnforcedValuesDeployed());
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn& txin : wtx.tx->vin)
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, -wallet->GetDebit(txin, ISMINE_ALL)) + "<br>";
            for (const CTxOut& txout : wtx.tx->vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + ProcyonUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal procyon:URI (procyon:123...?message=example)
...
QString TransactionDesc::toAssetHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if (nAssetsRec > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + ProcyonUnits::formatWithCustomName(QString::fromStdString(rec->assetName), nAssetsRec, rec->units) + "<br>";
    } else {
        ...
        if (wallet->IsMine(txin)) {
            ...
            if (assetData.nAmount > 0) {
                strHTML += "<b>" + tr("Debit") + ":</b> " +
                           ProcyonUnits::formatWithCustomName(QString::fromStdString(assetData.assetName), -assetData.nAmount) + "<br>";
            }
            strHTML += "<b>" + tr("Debit") + ":</b> " +
                       ProcyonUnits::formatHtmlWithUnit(unit, -debit) + "<br>";
        }
    ...
    for (const CTxOut& txout : wtx.tx->vout)
        if (wallet->IsMine(txout)) {
            if (txout.scriptPubKey.IsAssetScript()) {
                ...
                strHTML += "<b>" + tr("Credit") + ":</b> " +
                           ProcyonUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
        ...
        {
            {
                ...
                strHTML = strHTML + " " + tr("Amount") + "=" + ProcyonUnits::formatHtmlWithUnit(unit, vout.nValue);
