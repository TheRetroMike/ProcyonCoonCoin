...
const std::string ProcyonGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ProcyonGUI::DEFAULT_WALLET = "~Default";
...
ProcyonGUI::ProcyonGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ProcyonGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Procyon address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and procyon: URIs)"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    */
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Procyon addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Procyon addresses"));
    ...
    openAction->setStatusTip(tr("Open a procyon: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Procyon command-line options").arg(tr(PACKAGE_NAME)));
...
void ProcyonGUI::createToolBars()
{
    if(walletFrame)
    {
        /** RVN START */
...
//        toolbar->addAction(restrictedAssetAction);
        ...
        QVBoxLayout* procyonLabelLayout = new QVBoxLayout(toolbarWidget);
        procyonLabelLayout->addWidget(label);
        procyonLabelLayout->addWidget(toolbar);
        procyonLabelLayout->setDirection(QBoxLayout::TopToBottom);
        procyonLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("ProcyonCoin Market Price"));
...
void ProcyonGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ProcyonGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ProcyonGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ProcyonGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ProcyonGUI::setWalletActionsEnabled(bool enabled)
{
...
void ProcyonGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ProcyonGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ProcyonGUI::optionsClicked()
{
...
void ProcyonGUI::showDebugWindow()
{
...
void ProcyonGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ProcyonGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void ProcyonGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ProcyonGUI::openClicked()
{
...
void ProcyonGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ProcyonGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ProcyonGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ProcyonGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ProcyonGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ProcyonGUI::gotoVerifyMessageTab(QString addr)
{
...
void ProcyonGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Procyon network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ProcyonGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ProcyonGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ProcyonGUI::updateHeadersSyncProgressLabel()
{
...
void ProcyonGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ProcyonGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Procyon"); // default title
...
void ProcyonGUI::changeEvent(QEvent *e)
{
...
void ProcyonGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ProcyonGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ProcyonGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
...
void ProcyonGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ProcyonGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ProcyonGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ProcyonGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ProcyonGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void ProcyonGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ProcyonGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ProcyonGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ProcyonGUI::detectShutdown()
{
...
void ProcyonGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ProcyonGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ProcyonGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(ProcyonGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void ProcyonGUI::subscribeToCoreSignals()
{
...
void ProcyonGUI::unsubscribeFromCoreSignals()
{
...
void ProcyonGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ProcyonGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ProcyonUnits::Unit> units = ProcyonUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ProcyonUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(ProcyonUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(ProcyonUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (ProcyonUnits::Unit u : ProcyonUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ProcyonUnits::name(u)), this);
...
{
    setText(ProcyonUnits::name(newUnits));
}
...
void ProcyonGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ProcyonGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "ProcyonGUI::onPriceUnitChange: " + QString::number(newIndex);
