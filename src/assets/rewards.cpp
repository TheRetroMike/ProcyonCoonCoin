...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for PRCO
    ...
    if (p_rewardSnapshot.strDistributionAsset != "PRCO") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is PRCO with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using PRCO differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "PRCO") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Procyon address (already validated during ownership snapshot creation)
