...
const char *const PROCYON_CONF_FILENAME = "procyon.conf";
const char *const PROCYON_PID_FILENAME = "procyond.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Procyon
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Procyon
    // Mac: ~/Library/Application Support/Procyon
    // Unix: ~/.procyon
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Procyon";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Procyon";
#else
    // Unix
    return pathRet / ".procyon";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath)
{
    ...
    if (!streamConfig.good())
        return; // No procyon.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override procyon.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", PROCYON_PID_FILENAME));
