...
The Procyon Core daemon can be configured to act as a trusted "border
router", implementing the procyon wire protocol and relay, making
...
The ZeroMQ feature in Procyon Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building procyond:
...
For instance:
    ...
    $ procyond -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/procyond.tx.raw
...
These options can also be provided in procyon.conf.
...
From the perspective of procyond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into procyond directly. Furthermore, no information is
...
using. Procyond appends an up-counting sequence number to each
