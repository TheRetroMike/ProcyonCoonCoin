...
ProcyonCoin (PRCO) is a Proof of Work coin built on the Bitcoin UTXO model. As with other Bitcoin derivatives, PRCO coins are distributed to persons augmenting the Procyon network by mining Procyon.
...
The PRCO used to issue assets will be sent to a burn address, which will reduce the amount of PRCO available. 

Asset transfers require the standard PRCO transaction fees for transfer from one address to another.
...
Reward capabilities will be added to allow payment (in PRCO) to all holders of an asset. Payments of PRCO would be distributed to all asset holders pro rata. This is useful for paying dividends, dividing payments, or rewarding a group of token holders.

Example: A small software company issues an asset GAMECO that represents a share of the project. GAMECO tokens can be traded with others. Once the software company profits, those profits can be distributed to all holders of GAMECO by sending the profits (via PRCO) to all holders of GAMECO.
...
Rewards allow payment in PRCO to asset holders.

[More on rewards...](./rewards/README.md)

### Phase 4 - Unique Assets (Complete)

Once created, assets can be made unique for a cost of 5 PRCO. Only non-divisible assets can be made unique. This moves an asset to a UTXO and associates a unique identifier with the txid. From this point the asset can be moved from one address to another and can be traced back to its origin. Only the issuer of the original asset can make an asset unique.  
...
Voting will be accomplished by creating and distributing parallel tokens to token holders. These tokens can be sent to PRCO addresses to record a vote.

[More on voting...](./voting/README.md)

### Phase 7 - Compatibility Mode

Allows newly created assets to appear exactly like PRCO, LTC, or Bitcoin for easy integration into exchanges, wallets, explorers, etc.
