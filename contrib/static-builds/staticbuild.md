...
sudo find ${DISTNAME}/bin -type f -executable -exec ../procyoncoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../procyoncoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo procyon-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find procyon-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/procyoncoin
...
cp -f procyon-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf procyoncoin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/procyoncoin/procyon-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/procyon-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer procyon-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer procyon-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf procyoncoin/contrib/windeploy ~/sign/win32
...
#transfer procyon-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/procyoncoin/contrib/macdeploy/extract-osx-sdk.sh
...
cd ~/procyoncoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Procyon-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/procyon-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer procyon-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
