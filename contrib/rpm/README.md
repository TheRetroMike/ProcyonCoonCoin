...
The RPM spec file provided here is for Procyon-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://procyon.org/bin/ and
...
Currently only `Source10` is used, for the example `procyon.conf` file.
...
in the procyon git repository. At present only an SVG version of the Procyon
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the procyon
...
needed when 0.12.1 is released, a proper fix is already in the Procyon git
master branch.
...
idea to build Procyon Core against a static version of that library compiled
...
The `procyon.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `procyon-server` package. At this point, most RPM based
...
The `procyon-server` package should have SELinux support. How to properly do
...
Procyon-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/procyon/). He has been
packaging Procyon for Fedora at least since 2012.
...
4. I renamed the `procyon` package that contains the Qt GUI to `procyon-core` as
that appears to be how the general population refers to it, in contrast to
`procyon-xt` or `procyon-classic`. I wanted to make sure the general population
